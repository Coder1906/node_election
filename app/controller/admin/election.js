...
class ElectionController extends Controller {
  ...
  async list() {
    ...
    if (RegEx.checkUint(status)) query.status = Number(status);
  ...
  async add () {
    ...
    if (!RegEx.checkName(name) || !RegEx.checkDateTime(start) || !RegEx.checkDateTime(end) || !RegEx.checkUint(status)) {
  ...
  async update () {
    ...
    let electionId = Number(ctx.params.id);
    let election = await ctx.service.election.getById(electionId);
    if (!election) {
      ctx.body = {code: -2, msg: '该记录不存在'};
      return
    }
    let result = await ctx.service.election.update(electionId, {name, start, end, status: Number(status)});
    ctx.body = result? {code: 1}: {code: 0, msg: '服务器内部错误'};
    return
  }
}
